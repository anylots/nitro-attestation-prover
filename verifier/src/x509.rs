...
pub fn compute_path_digests(certs_der: &[&[u8]]) -> Vec<B256> {
    ...
    for (i, der) in certs_der.iter().enumerate() {
        let cert_digest = B256::from_slice(&Sha256::digest(der));
        if i == 0 {
            ...
            path_digest = B256::from_slice(&hasher.finalize());
        }
...
mod tests {
    ...
    fn duplicate_root_rejected(full_chain_der: Vec<Vec<u8>>, #[case] trusted_prefix_len: usize) {
        ...
        let chain = CertChain::from_der(&refs).unwrap();
        let err = chain
            .verify_chain(trusted_prefix_len, VALID_TIMESTAMP_MS)
            .unwrap_err();

        assert!(
            err.to_string().contains("self-issued"),
    ...
    fn trailing_der_certificate_alias_rejected(full_chain_der: Vec<Vec<u8>>) {
        ...
        assert!(
            msg.contains("certificate 1"),
            "expected cert index in error, got: {msg}"
        );
        assert!(
            msg.contains("trailing DER data"),
            "expected trailing DER error, got: {msg}"
        );
    }
